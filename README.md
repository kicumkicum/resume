...
- Разработка VK Teams бота для доступа к ChatGPT. Придумал, запрограммировал, развернул на сервере в docker контейнере с маршрутизацией через proxy-сервер. Настроил автобилд и автодеплой. Для реализации на typescript, с помощью LLM сконвертировал python sdk vk teams в версию на typescript;
